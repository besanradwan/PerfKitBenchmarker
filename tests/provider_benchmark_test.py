...
class ProviderBenchmarkChecks(unittest.TestCase):
  ...
  def testPingSupported(self):
    for cloud in benchmark_spec.VALID_CLOUDS:
      ...
      ThisProviderInfoClass = provider_info.GetProviderInfoClass(cloud.lower())
  ...
  def testMYSQLSupport(self):
    for cloud in benchmark_spec.VALID_CLOUDS:
      providers.LoadProvider(cloud.lower())
      ThisProviderInfoClass = provider_info.GetProviderInfoClass(cloud.lower())
      if (cloud.lower() == 'aws' or cloud.lower() == 'gcp'):
        self.assertTrue(ThisProviderInfoClass.IsBenchmarkSupported('mysql'))
      else:
        self.assertFalse(ThisProviderInfoClass.IsBenchmarkSupported('mysql'))
